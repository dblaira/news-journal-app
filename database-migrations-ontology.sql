...
-- -----------------------------------------------------------------------------
-- 6. Seed global axioms (user_id NULL — run in SQL editor; idempotent by name)
-- -----------------------------------------------------------------------------
INSERT INTO ontology_axioms (user_id, name, description, antecedent, consequent, confidence, sources)
SELECT NULL, 'Learning Master Key', 'Learning strongly predicts positive affect, ambition, and insight', 'High Learning', '↑ Affect (67%), ↑ Ambition (65%), ↑ Insight (62%)', 0.67, ARRAY['journal', 'youtube']::TEXT[]
WHERE NOT EXISTS (SELECT 1 FROM ontology_axioms o WHERE o.user_id IS NULL AND o.name = 'Learning Master Key');

INSERT INTO ontology_axioms (user_id, name, description, antecedent, consequent, confidence, sources)
SELECT NULL, 'Exercise-Sleep Synergy', 'High exercise combined with high sleep creates excellent stress recovery', 'High Exercise + High Sleep', 'StressRecoveryPattern_Excellent', 0.57, ARRAY['apple_health', 'journal']::TEXT[]
WHERE NOT EXISTS (SELECT 1 FROM ontology_axioms o WHERE o.user_id IS NULL AND o.name = 'Exercise-Sleep Synergy');

INSERT INTO ontology_axioms (user_id, name, description, antecedent, consequent, confidence, sources)
SELECT NULL, 'Belief to Entertainment Lag', 'High belief activity predicts higher entertainment two weeks later', 'High Belief week N', '↑ Entertainment week N+2', 0.60, ARRAY['journal']::TEXT[]
WHERE NOT EXISTS (SELECT 1 FROM ontology_axioms o WHERE o.user_id IS NULL AND o.name = 'Belief to Entertainment Lag');

INSERT INTO ontology_axioms (user_id, name, description, antecedent, consequent, confidence, sources)
SELECT NULL, 'Zero Negative Impact', 'No life domain shows strong negative correlation with any other', 'Any Domain High', 'No Strong Negative Impact on Other Domains', 0.95, ARRAY['all_sources']::TEXT[]
